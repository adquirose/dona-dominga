...
function Formulario(){
  ...
  const handleOnSubmit = (event) => {
    event.preventDefault();
		...
		const { nombre, email, rut, telefono } = data 
    const message = `
			Nombre: ${nombre} 
			Email: ${email} 
			Telefono: ${telefono}
    ...
    if(validateRut(rut)){
      const info = {
        to:'info@parcelaciondonadominga.cl , ventas@parcelaciondonadominga.cl , gerencia@parcelaciondonadominga.cl' ,
        replyTo: email,
  ...
  const { nombre, email, rut, telefono } = data
  const isDisabled = nombre === '' || email === '' || rut === '' || telefono === '' 
